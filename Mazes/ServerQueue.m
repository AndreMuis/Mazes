...
@property (strong, nonatomic) MAEvent *enqueueOldestObjectEvent;
@property (strong, nonatomic) NSMutableArray *pendingObjects;
...
- (id)init
{
	...
	if (self)
	{
        ...
        _enqueueOldestObjectEvent = [[MAEvent alloc] initWithTarget: self
                                                             action: @selector(enqueueOldestObject)
                                                       intervalSecs: [Constants shared].serverRetryDelaySecs
                                                            repeats: NO];
        
        _pendingObjects = [[NSMutableArray alloc] init];
	}
...
- (void)serverOperationsSaveMazeUserWithError: (NSError *)error
{
    if (error == nil)
    {
        [[MainListViewController shared] savedMazeUser: [self.pendingObjects lastObject]];
...
- (void)serverOperationsSaveRatingWithError: (NSError *)error
{
    if (error == nil)
    {
        [[MainListViewController shared] savedRating: [self.pendingObjects lastObject]];
